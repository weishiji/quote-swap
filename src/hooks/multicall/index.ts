...
function useCallsData(
  ...
  const callResults = useSelector((state: TRootState) => state.multicall.callResults, shallowEqual);
...
export function useSingleCallResult(
  ...
  return (
    useSingleContractMultipleData(contract, methodName, [inputs], options)[0] ?? INVALID_CALL_STATE
  );
}
